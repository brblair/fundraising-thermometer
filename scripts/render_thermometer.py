...
def main():
    ...
    for i, seg_val in enumerate(segs):
        bar_x=start_x+i*(bar_w+gap); pct=seg_val/SEG_GOAL; fill_h=int(bar_h*pct); fill_y=bar_y+(bar_h-fill_h); color=progress_color(pct)
        ...
        svg.append(f'<circle cx="{bulb_cx}" cy="{bulb_cy}" r="{bulb_r-4}" fill="{color}"/>' )
